...
protected:
    int _numSeats;
public:
    Vehicle(int numSeats);// may serve as default ctor (i.e. no arguments)
    virtual ~Vehicle();
...
Car::Car(int numSeats, int maxWeight) : Vehicle(numSeats) {
...
Truck::Truck(int numSeats, int payload, double volume) : Vehicle(numSeats) {
    _numSeats = numSeats;
    _payload = payload;
    _volume = volume;
}
