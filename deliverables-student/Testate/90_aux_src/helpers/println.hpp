...
// H_PRETTY_PRINT is set if you #include "cxx-prettyprint/prettyprint.hpp" before #include "println.hpp"
...
inline std::string as_string<unsigned int>(unsigned int i){
  return std::string("\033[1m") + to_string_with_precision(i) + std::string("\033[0m");
}

template <>
inline std::string as_string<unsigned long>(unsigned long i){
  return std::string("\033[1m") + to_string_with_precision(i) + std::string("\033[0m");
}

template <>
inline std::string as_string<signed long long int>(signed long long int i){
  std::string retval = std::string("\033[1m");
  if(i>=0){
    retval += '+';
  }
...
template <>
inline std::string as_string<signed long>(signed long i){
  std::string retval = std::string("\033[1m");
  if(i>=0){
    retval += '+';
  }
...
template <>
inline std::string as_string<float>(float f){
  return std::string("\033[4m") + to_string_with_precision(f) + std::string("\033[0m");
}
...
template <>
inline std::string as_string<void *>(void * p){
    std::ostringstream out;
    out << std::hex << std::setw(16) << std::setfill('0') << std::internal << p;
    return out.str();
}
...
inline void println(const T& t, const U& ...u)
{
